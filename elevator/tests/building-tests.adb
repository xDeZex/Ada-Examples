...
package body Building.Tests is
   ...
   begin
      Assert (Manager.Elevator1.Which_Floor_Is_Elevator_On = 1, "Elevator starts on wrong floor");

      Manager.Elevator1.Move_Elevator_To_Floor (2);

      Assert (Manager.Elevator1.Which_Floor_Is_Elevator_On = 2, "Elevator moves to wrong floor");
   ...
   procedure Is_The_Correct_Amount_of_People_On_The_Elevator (T : in out Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
   begin
      Assert (Manager.Elevator1.How_Many_People_In_Elevator = 0, "Elevator start with wrong number of people in it");
      
      Manager.Elevator1.Person_Enter_Elevator;

      Assert (Manager.Elevator1.How_Many_People_In_Elevator = 1, "Person doesn't enter elevator");
   ...
   begin
      ...
      Manager.Elevator1.Person_Leave_Elevator;

      Assert (Manager.Elevator1.How_Many_People_In_Elevator = 0, "Person doesn't leave elevator");
      Assert (Manager.Elevator1.How_Many_People_Are_On_The_Correct_Floor = 1, "Person leaves elevator but isn't on correct floor");
   ...
   begin
      Manager.Elevator1.Person_Enter_Elevator;
      Manager.Elevator1.Move_Elevator_To_Floor (3);

      Manager.Elevator1.Person_Leave_Elevator;
      
      Assert (Manager.Elevator1.How_Many_People_In_Elevator = 1, "Person leaves elevator on wrong floor");
      Assert (Manager.Elevator1.How_Many_People_Are_On_The_Correct_Floor = 0, "Person doesn't leave elevator but is on correct floor");
   ...
   begin
      ...
      Assert (Manager.Elevator1.How_Many_People_In_Elevator = 0, "Person enters elevator on wrong floor");
   ...
   procedure Does_Elevator_Show_Where_People_Are_Going (T : in out Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
   begin
      Assert (Manager.Elevator1.Where_People_Are_Going_From_Floor (1) (0) = 2, "Doesn't show where people are going correctly on floor 1");
      Assert (Manager.Elevator1.Where_People_Are_Going_From_Floor (1)'Length = 1, "Returned array is not the right length on floor 1");

      Assert (Manager.Elevator1.Where_People_Are_Going_From_Floor (3) (0) = 4, "Doesn't show where person 1 are going correctly on floor 3");
      Assert (Manager.Elevator1.Where_People_Are_Going_From_Floor (3) (1) = 1, "Doesn't show where person 2 are going correctly on floor 3");
      Assert (Manager.Elevator1.Where_People_Are_Going_From_Floor (3)'Length = 2, "Returned array is not the right length on floor 3");
   ...
   begin
      Person := (2, 1, 1);
      Manager.Elevator1.Add_Person (Person, 1);
      Person := (4, 3, 3);
      Manager.Elevator1.Add_Person (Person, 3);
      Person := (1, 3, 3);
      Manager.Elevator1.Add_Person (Person, 3);
